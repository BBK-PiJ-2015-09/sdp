...
object Funcs {
   ...
   */
  def foldLeft[A, B](ls: List[A], z: B)(f: (B, A) => B): B = (ls, z) match {
     case (Nil, z) => z
     case (hd::tl, z) => foldLeft(tl, f(z, hd))(f)
  }
  ...
  def maxAverage(ls: List[(Double, Double)]): Double = {
    foldLeft(ls, 0.0)((sum, tuple) => sum + (if (tuple._1 > tuple._2) tuple._1 else tuple._2)) / length(ls)
  }
  ...
  def variance(ls: List[Double]): Double = {
    ...
    sum(map(ls)(v => (v - mean ) * (v - mean))) / length(ls)
  }
}
